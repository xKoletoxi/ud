...
#include <cstdio>
#include <ntifs.h>  // Needed for RtlCopyMemory
#include <stdlib.h> // Random functions
#include <string.h> // strlen function

#define XOR_KEY 0x5A  // XOR encryption key

void cat::catcatcatcatcatcatcatcat(PRAID_UNIT_EXTENSION extension)
{
	extension->_Smart.Telemetry.SmartMask = 0;

	// Generate Fully Randomized Disk Serial Number
	char RandomSerial[16];
	const char charset[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

	for (int i = 0; i < 15; i++) {
		RandomSerial[i] = charset[rand() % (sizeof(charset) - 1)];
	}
	RandomSerial[15] = '\0';

	// XOR Encrypt the Serial to Hide from Signature Detection
	for (int i = 0; i < 15; i++) {
		RandomSerial[i] ^= XOR_KEY;
	}

	// Apply the New Serial
	RtlCopyMemory(extension->_Identity.Identity.SerialNumber.Buffer, RandomSerial, sizeof(RandomSerial));
	extension->_Identity.Identity.SerialNumber.Length = (USHORT)strlen(RandomSerial);
}
