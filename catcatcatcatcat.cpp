...
PVOID cat::catcatcatcatcatcatcatcatcatcatcat(const char* cattttttttttttttttttttt)
{
	...
	// Dynamically resolve ZwQuerySystemInformation in kernel mode
	...
	auto status = pZwQuerySystemInformation(SystemModuleInformation, &size, 0, &size);
	...
	status = pZwQuerySystemInformation(SystemModuleInformation, moduleList, size, nullptr);
